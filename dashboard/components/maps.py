...
def load_latest_model():
    ...
    try:
        ...
        st.error(f"Error loading model: {str(e)}. The model was likely trained with a different Python version. Please delete models/*.pkl and retrain using: python src/train_model.py")
        return None
...
def load_latest_data():
    ...
    try:
        ...
        if df.empty:
            st.warning("Dataset is empty. Please collect data using the steps in SETUP.txt")
            return None
        return df
    except Exception as e:
        st.error(f"Error loading data: {str(e)}. Please ensure data has been collected and processed.")
        return None
...
if section == "Overview":
    ...
    st.info("""
    **First time setup:** This project requires data collection before the dashboard becomes fully functional.
    
    Please follow the setup guide in `SETUP.txt` to:
    1. Set up your API keys (VTA_API_KEY and OPENWEATHER_API_KEY)
    2. Collect transit and weather data
    3. Process and merge the data
    4. Train the prediction model
    
    Once complete, the Model Performance and Data Preview sections will display your results.
    """)
